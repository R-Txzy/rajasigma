...
module.exports = async (jid, client) => {
   ...
   if (credsExists && !fs.existsSync(`./${env.bot_hosting.session_dir}/${sessionPath}/creds.json`)) {
      ...
      connect.on('group.add', async ctx => {
         const sock = connect.sock
      ...
      connect.on('group.remove', async ctx => {
         const sock = connect.sock
