const Bingo = require('lib/system/bingo')
exports.run = {
   usage: ['bingo'],
   category: 'games',
   async: async (m, {
      ...
      const words = wordFinder.getRandomElements(require('media/json/kbbi').filter(v => v.length >= 4).filter(v => v.length <= 5), 5).map(v => v.toUpperCase())
      ...
      client.bingo[id] = [
         ...
         setTimeout(() => {
            if (client.bingo[id]) {
               ...
               teks += isJson.answer.map((v, i) => (i + 1) + '. ```' + Func.ucword(v) + '```').join('\n')
