   ...
   async: async (m, {
      ...
      try {
         ...
         if (groupSet.antilink && !isAdmin && body) {
            if (body.match(/(chat.whatsapp.com)/gi) && body.match(/(whatsapp.com)/gi) && !body.includes(await client.groupInviteCode(m.chat)) || body.match(/(wa.me)/gi)) return client.sendMessage(m.chat, {
               delete: {
                  remoteJid: m.chat,
                  fromMe: false,
                  id: m.key.id,
