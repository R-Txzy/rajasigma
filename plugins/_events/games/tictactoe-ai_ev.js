const TicTacToe = require('lib/system/tictactoe-ai')
exports.run = {
   async: async (m, {
      client,
      body,
      ...
      try {
         ...
         const id = m.sender
         const reward = Func.randomInt(1500, 10000)
               ...
               if (game.checkWinner() || game.isBoardFull()) {
                  ...
                  if (session.moves.winner.winner) {
                     if (session.moves.winner.winner === 'Player') {
                        users.point += reward
                        str += `> Congratulations @${id.replace(/@.+/, '')}, you win! +${Func.formatter(reward)} Points`
                     } else {
                  ...
                  m.reply(str.trim()).then(() => {
                     ...
                     Func.removeItem(client.ttt, session)
                  })
                  return
               }
