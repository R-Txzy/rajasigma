exports.run = {
   ...
   async: async (m, {
      ...
      try {
         ...
         caption += `	◦  *Game Limit* : ${Func.formatNumber(target.limit_game)}\n`
         caption += `	◦  *Level* : ${Func.level(target.point, env.multiplier)[0]} (${Func.role(Func.level(target.point, env.multiplier)[0])})\n`
         ...
         caption += `	◦  *Banned* : ${(new Date - target.ban_temporary < env.timeout) ? Func.toTime(new Date(target.ban_temporary + env.timeout) - new Date()) + ' (' + ((env.timeout / 1000) / 60) + ' min)' : target.banned ? '√' : '×'}\n`
