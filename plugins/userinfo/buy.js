   ...
   async: async (m, {
      ...
      try {
         ...
         if (command == 'buyall') {
            ...
            if (users.limit_game >= maximum) return client.reply(m.chat, Func.texted('bold', `🚩 Sorry, you can't buy any more game limits because you have reached maximum limit.`), m)
            if (isNaN(args[0])) return client.reply(m.chat, Func.example(isPrefix, command, '1'), m)
            if (args[0] < 1) return client.reply(m.chat, Func.example(isPrefix, command, '1'), m)
            if (users.point >= price * parseInt(args[0])) {
               if ((users.limit_game + parseInt(args[0])) >= maximum) return client.reply(m.chat, Func.texted('bold', `🚩 Game Limit amount you buy exceeds maximum limit.`), m)
               users.point -= price * parseInt(args[0])
               users.limit_game += parseInt(args[0])
               return client.reply(m.chat, `✅ You have purchased *${args[0]}* game limit with *${Func.h2k(price * args[0])}* points.`, m)
            } else {
               client.reply(m.chat, Func.texted('bold', `🚩 You don't have enough points to buy ${Func.formatNumber(args[0])} limit.`), m)
            }
